...
export const computeBucketsPercentages = (
    buckets: Omit<Bucket, 'percentage'>[],
    total: number
) => {
    ...
    const percentageTotal = round(bucketsWithPercentage.reduce((acc, bucket) => acc + bucket.percentage, 0))
